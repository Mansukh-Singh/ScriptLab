...
import { change, revert } from '@/redux/features/counter/usernameSlice'
import './login.css'
...
const page = () => {
    ...
    const SignIn = async () => {
        ...
        const handleAlert = async () => {
            alert_div.current.style.transition = "all 0.3s ease"
            alert_div.current.style.top = "-128px"
            await wait(300)
            setAlert(false)
            setApiresult(null)
        }
        ...
        return (
            <>
                {alert && <div ref={alert_div} className="relative z-20 m-auto flex shadow-lg rounded-lg gap-2 justify-between bg-yellow-700 text-white items-center top-[-128px] w-80 h-14 text-md font-mono font-semibold">
                ...
                <div className='relative z-[0] overflow-hidden shadow-lg flex flex-col justify-items items-center loginboard w-[30vw] h-[60vh] m-auto mt-20 rounded-3xl  shadow-black'>
                    {hiddendiv && <div className="absolute flex flex-col z-[7] justify-center items-center gap-3 rounded-3xl top-0 left-0 w-[30vw] h-[60vh]">
                        <span className="fog_pass flex justify-center items-center text-3xl font-semibold shadow-md shadow-black px-3 py-1 rounded-lg">Forgot Password</span>
                        ...
                        <input type="text" className='email w-[25vw] p-1 font-mono font-bold shadow-md shadow-black' placeholder="Enter your Email ID" />
                        <button ref={hiddensubmitButton} onMouseUp={hiddenbuttonUp} onMouseDown={hiddenbuttonDown} className="relative flex justify-center items-center w-[25vw] h-[4vh] border border-black bg-none font-mon font-semibold py-4 rounded-md shadow-md shadow-black">Start password reset</button>
